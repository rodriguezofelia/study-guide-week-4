...
SELECT models.name, brands.name,  headquarters FROM models JOIN brands USING (brand_id) WHERE models.name = 'Mustang'; 
...
SELECT * FROM models WHERE model_id  >= 15 AND model_id <= 24 ORDER BY year  LIMIT 10;
...
SELECT models.year, models.name, model_id, awards.name FROM models LEFT JOIN awards ON models.model_id = awards.winner_id WHERE models.year >= 1964;
...
SELECT brands.name, models.year, models.name FROM brands JOIN models ON brands.brand_id = models.brand_id ORDER BY brands.name;
...
SELECT brands.name, models.name, models.year, brands.discontinued, brands.discontinued - models.year AS years_until_brand_discontinued FROM models LEFT JOIN brands ON models.brand_id = brands.brand_id WHERE brands.discontinued IS NOT NULL ORDER BY brands.name, models.name, models.year; 
...
SELECT brands.name, brands.founded FROM brands LEFT JOIN models ON models.brand_id = brands.brand_id WHERE models.model_id IS NULL; 
...
SELECT name, year FROM models WHERE year IN (SELECT founded FROM brands WHERE founded = year);